...
( function ( $, window, document, undefined ) {
	...
	function scrollToStep( stepParam, fromAnimation ) {
		...
		if( scrollHandle )
		    {
			scrollHandle.css( "top", stepParam / (pathList.length - 1 ) * ( scrollBar.height() - scrollHandle.height() ) + "px" );
			var color = rainbow(stepParam/(pathList.length-1));
			console.log(color);
		     $(".sp-scroll-handle").css("background",color);}
	...
	*/
    ...
    function rainbow(n){
	// This function maps a decimal 0~1 to red, orange ... purple
	var r = 0;
	var g = 0;
	var b = 0;
	// Determine red
	if (n<=0.3) { r = 255;}
	    ...
	    if (n<=0.5) { r = 255 - 255*(n-0.3)/0.2;}
	    else { if(n<=0.9){r = 0;}
		   else {r = 255*(n-0.9)/0.1;}
		 }
	}
	// Determine blue
	if (n<=0.5) {b = 0;}
	else {
	    if (n<=0.7) {b=255*n-0.5/0.2;}
	    else { b = 255; }
	}
	// Determine green
	if (n<=0.3) {g=255*n/0.3;}
	else {
	    if (n<=0.7) {g=255;}
	    else {
		g = 255 - 255*(n-0.7)/0.3;
	...
	// Now we have r, g, b in the range (0, 255)
	var rs = (parseInt(r)).toString(16);
	var gs = (parseInt(g)).toString(16);
	var bs = (parseInt(b)).toString(16);
